...
export default function EditLeavePage({ params }: { params: Promise<{ id: string }> }) {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    if (data.status !== "Pending" && data.status !== "Returned") {
                        alert("Cannot edit leave requests that are not Pending or Returned.");
                        router.push("/hod/history");
                        return;
                    }
                    setFormData({
    ...
    if (loading) {
        return <DashboardLayout allowedRole="hod"><div className="p-10 text-center">Loading...</div></DashboardLayout>;
    }
