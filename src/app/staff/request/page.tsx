...
export default function LeaveRequestPage() {
        ...
        const fetchBalance = async () => {
            try {
                ...
                const totalUsed = usedSnap.docs
                    .filter(d => {
                        const data = d.data();
                        return data.status !== "Rejected" && (data.fromDate || "") >= "2026-03-25";
                    })
    ...
    // Fetch leave balance for the currently selected (non-comp) leave type
    useEffect(() => {
        if (!user || isCompLeave || formData.type === "Loss of Pay") { setLeaveBalance(null); return; }
    ...
    const getAvailableSessions = (dateString: string) => {
        let allSessions = ["Full Day", "Forenoon", "Afternoon"];

        if (formData.type === "Compensatory Leave" || formData.type === "Vacation Leave") {
            allSessions = ["Full Day"];
        }
        ...
        if (isToday(dateString)) {
            ...
            if (currentMinutes > limit830) {
                return allSessions.filter(s => s === "Afternoon");
            }
        }
        return allSessions;
    };
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    const isFullDayOnly = newType === "Compensatory Leave" || newType === "Vacation Leave";
                                    setFormData({ 
                                        ...formData, 
                                        type: newType,
                                        session: isFullDayOnly ? "Full Day" : formData.session,
                                        toDate: (!isFullDayOnly && formData.session !== "Full Day") ? formData.fromDate : formData.toDate
                                    });
                                }}
                            >
