...
function StaffDashboardContent() {
    ...
    useEffect(() => {
        if (userData && !loading) {
            const hasQuals = userData.qualifications && userData.qualifications.length > 0;
            const hasExp = userData.experiences && userData.experiences.length > 0;
            if (!hasQuals || !hasExp) {
            ...
            <div className="space-y-6 relative">
                ...
                {showProfilePrompt && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
                        <div className="bg-white rounded-2xl p-6 shadow-xl max-w-md w-full animate-in zoom-in-95 duration-200">
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                    <div className="p-2 bg-blue-100 text-blue-600 rounded-full">
                                        <AlertCircle className="h-6 w-6" />
                                    </div>
                                    <h3 className="text-xl font-bold text-gray-900">Incomplete Profile</h3>
                                </div>
                                <button onClick={handleDismissProfilePrompt} className="text-gray-400 hover:text-gray-600">
                                    <X className="h-5 w-5" />
                                </button>
                            </div>
                            <p className="text-gray-600 mb-6">
                                Your profile is missing Qualifications or Experience details. Please update your profile to keep our records complete.
                            </p>
                            <div className="flex justify-end gap-3">
                                ...
                                <button 
                                    onClick={() => router.push('/staff/complete-profile')}
                                    ...
                                    <div key={type} className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                                        <div className="flex justify-between items-center mb-2">
                                            ...
                                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isNearLimit ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                                                {type === "Loss of Pay" ? `${used} Used` : `${used}/${effectiveLimit}`}
                                            </span>
                                        </div>
                                        {type !== "Loss of Pay" && (
