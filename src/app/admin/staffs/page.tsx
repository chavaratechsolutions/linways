                ...
                <div className="grid grid-cols-1 gap-4 md:hidden">
                    {loading ? (
                        ...
                        sortedAndFilteredStaffs.map((staff) => (
                            <Link href={`/admin/staffs/${staff.id}`} key={staff.id} className="block">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {loading ? (
                                    ...
                                    sortedAndFilteredStaffs.map((staff, index) => (
                                        <tr key={staff.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => window.location.href = `/admin/staffs/${staff.id}`}>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium uppercase ${staff.role === 'princi' ? 'bg-purple-100 text-purple-800' :
                                                    staff.role === 'dir' ? 'bg-orange-100 text-orange-800' :
                                                        staff.role === 'hod' ? 'bg-indigo-100 text-indigo-800' :
